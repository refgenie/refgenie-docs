...
The pre-submit hook that fills the `refgenie` namespace has a new name:
...
It reads the database from `$REFGENIE_DB_CONFIG_PATH`, or from
`var_templates.refgenie_db_config` in the pipeline interface when that is set.
...
Legacy plugins registered functions in `refgenie.hooks.<hook>` entry-point
groups, and refgenie1 keeps that scheme. What changed is the hook list, the
function signature, and where a plugin keeps its settings.

| Legacy hook | refgenie1 |
| --- | --- |
| `post_update` | `post_update`: once, after any operation that changed local assets, aliases or genomes |
| `pre_pull` / `post_pull` | same, once per asset pulled |
| `pre_update` | removed |
| `pre_tag` / `post_tag` | removed (tags are gone) |
| `pre_list` / `post_list` | removed (listing is a read) |
| (none) | new: `pre_build` / `post_build` |

A legacy entry point in a removed group is never called; `refgenie plugins`
shows it as `unknown hook (never fires)`.

A plugin used to be called as `func(rgc)`. It is now called as
`func(rg, event)`, where `rg` is the `Refgenie` instance and `event` is a
`refgenie.plugins.HookEvent`. A one-argument function fails with `TypeError`,
which refgenie logs with the plugin's name and moves on.

Before:

```python
def update_config(rgc):
    for genome, assets in rgc.list_assets_by_genome().items():
        for asset in assets:
            print(genome, asset, rgc.seek(genome, asset))
```
...
def update_config(rg, event):
    paths = rg.paths()
    for genome in paths:  # genome aliases
        for group in paths[genome]:  # groups with a default asset
            path = rg.asset.seek_components(
                AssetRegistryPathComponents(genome=genome, asset_group=group)
            )
            print(genome, group, path)
```

Free-form keys in the config file (such as refgenie_nfcore's
`nextflow_config:`) are gone, because there is no config file. Plugin settings
live in the database instead. The user runs
`refgenie plugins set nfcore config_path=/abs/path/nf.config`, and the plugin
reads `rg.plugins.settings("nfcore")` inside its hook.

See the plugin author guide on refgenie.org
(<https://refgenie.org/refgenie/plugins/>).

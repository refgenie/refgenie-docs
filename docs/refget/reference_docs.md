...
High-performance sequence store with 2-bit encoding.

!!! note "Documentation Limitation"
    Autodoc for PyO3/Rust compiled extensions is not supported by mkdocstrings.
    Use `help(GlobalRefgetStore)` in Python for full API documentation.
    See [mkdocstrings_pyo3_problem.md](https://github.com/refgenie/refgenie-docs/blob/master/plans/mkdocstrings_pyo3_problem.md) for details.
...
# Or load remote
store = GlobalRefgetStore.load_remote("/cache", "https://example.com/store")

# Get sequences
seq = store.get_substring("digest", 0, 1000)
```

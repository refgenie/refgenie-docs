...
## Introduction
Once you've [installed refgenie](/legacy/install/), you can use `refgenie pull` to [download pre-built assets](/legacy/pull/) without installing any additional software. However, you may need to use the `build` function for genomes or assets that are not available on the server. You can build assets for any genome for which you can provide the required inputs.
...
You can see the [example build output](/legacy/build_output/).
...
You can also learn more about [tagging refgenie assets](/legacy/tag/).

...
Refgenie facilitates such fine-grained comparison of genomes via `refgenie compare` command. A useful "byproduct" of genome initialization described in [Use aliases how-to guide](/legacy/notebooks/aliases/) is a JSON file with annotated sequence digests, that are required to compare FASTA file contents.
